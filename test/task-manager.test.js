...
test("разбирает голосовую команду с приоритетом и сроком", () => {
  ...
  assert.equal(result.data.urgency, "medium");
...
test("синхронизирует Android-проект и задачу по правилу последнего изменения", () => {
  ...
  assert.deepEqual(first.applied, { projects: 1, tasks: 1, ideas: 0 });
  ...
  assert.deepEqual(stale.applied, { projects: 0, tasks: 0, ideas: 0 });
...
test("синхронизирует SQLite с Google Sheets API после OAuth", async () => {
    ...
    fetchImpl: async (url, options = {}) => {
      if (String(url).includes("values:batchGet")) {
        return new Response(JSON.stringify({ valueRanges: [
          { values: [] },
          { values: [[remoteId, "", "", "", "Задача из OAuth-таблицы", "", "todo", "high", "urgent", "", "", "ChatGPT", "2026-07-19T08:00:00.000Z", "2026-07-19T09:00:00.000Z", "Google Sheets", ""]] },
          { values: [] }
        ] }), { status: 200 });
      }
  ...
  assert.ok(written.data.find(item => item.range === "Tasks!A2:P").values.some(row => row[4] === "Локальная OAuth-задача"));
});

test("распределяет задачи по срочности относительно даты создания", () => {
  const { service } = setup();
  const urgent = service.processDialogue("Мне нужно позвонить клиенту сегодня в 15:30").data;
  const medium = service.processDialogue("Задача: отправить отчёт завтра").data;
  const later = service.processDialogue("Задача: обновить архив до 25.07.2026").data;

  assert.equal(urgent.urgency, "urgent");
  assert.equal(medium.urgency, "medium");
  assert.equal(later.urgency, "not_urgent");
});

test("сохраняет идеи отдельно от задач", () => {
  const { service } = setup();
  const result = service.processDialogue("Идея на будущее: создать библиотеку шаблонов");

  assert.equal(result.analysis.intent, "create_idea");
  assert.equal(result.data.status, "new");
  assert.equal(service.listIdeas().length, 1);
  assert.equal(service.listTasks().length, 0);
});

test("создаёт проектную задачу и при необходимости сам проект", () => {
  const { service } = setup();
  const result = service.processDialogue("Задача для проекта Компас: подготовить прототип до завтра");

  assert.equal(result.data.projectTitle, "Компас");
  assert.equal(result.data.urgency, "medium");
  assert.equal(service.listProjects()[0].title, "Компас");
});

test("интерфейс содержит подсветку и системные уведомления срочных задач", () => {
  ...
  assert.match(script, /new Notification\("Орбита: срок срочной задачи истёк"/u);
  assert.match(styles, /\.urgent-task/u);
});
